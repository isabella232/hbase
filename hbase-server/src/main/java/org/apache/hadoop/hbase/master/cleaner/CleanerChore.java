...
public abstract class CleanerChore<T extends FileCleanerDelegate> extends ScheduledChore
  ...
  private void preRunCleaner() {
    for(FileCleanerDelegate fcd: cleanersChain) {
      fcd.preClean();
    }
  }
  ...
  private class CleanerTask extends RecursiveTask<Boolean> {
    ...
    protected Boolean compute() {
      ...
      try {
        ...
        subDirs = getFilteredStatus(new Predicate<FileStatus>() {
          @Override
          public boolean apply(FileStatus fileStatus) {
            return fileStatus.isDirectory();
          }
        });
        files = getFilteredStatus(new Predicate<FileStatus>() {
          @Override
          public boolean apply(FileStatus fileStatus) {
      ...
      if (!subDirs.isEmpty()) {
        List<CleanerTask> tasks = new ArrayList(subDirs.size());
        ...
        final List<CleanerTask> t = tasks;
        allSubdirsDeleted = deleteAction(new Action<Boolean>() {
          @Override
          public Boolean act() throws IOException {
            return getCleanResult(t);
          }
        }, "subdirs");
      }
    ...
    private List<FileStatus> getFilteredStatus(final Predicate<FileStatus> function) throws IOException {
      List<FileStatus> fileStatuses = FSUtils.listStatusWithStatusFilter(fs, dir, new FileStatusFilter() {
        @Override
        public boolean accept(FileStatus f) {
          return function.apply(f);
        }
      });
      if(fileStatuses == null) fileStatuses = Collections.emptyList();
      return fileStatuses;
    }
